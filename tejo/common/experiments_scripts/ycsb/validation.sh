...
if [ $# -eq 1 ]; then
	...
	workload_file=${YCSB_HOME}/workloads/$label
		...
		echo "invalid number of parameters, expected a single additional parameter or none. bye"
		exit -1
fi

validation_file=/tmp/my_validation.txt

echo -n $system_id > /tmp/slo_system_id.txt


for threads in 10 12 14 16 18 20 22 24 28 32 36 40 44 48 50 64 100 128 200 256 512
do
	...
	do
		...
		now=`date`
		ts=`date +"%s"`
		echo "" >> /tmp/my_validation.txt
		echo -n "${ts},$threads,$max_conn">> /tmp/my_validation.txt
