...
#    worload_peers={}
    ...
    #1.1) 
    for peer in peers:
        if remaining_operation_tokens > 0 and peer['peer'] not in ple_nodes:
            print "%s: %s" % (peer['peer'],str(peer['peer'] not in ple_nodes))
    ...
    for peer in peer_to_update:
        ...
        measured_nodes=measured_nodes+1
        print '(%d/%d)%s:%.4f (rtt) , %d (number of monitors) [UPDATED]'%(measured_nodes,operation_tokens,peer,new_peers[peer],len(peer_to_update[peer]['monitors']))
